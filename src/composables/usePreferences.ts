...
export const usePreferences = ()=>{
    ...
    const initialThemePref:string = localStorage.getItem(LocalStorage.Theme) || global.name.value;
    const initialCurrencyPref:string = localStorage.getItem(LocalStorage.Currency) || import.meta.env.VITE_DEFAULT_CURRENCY;
    const initialLangPref:string = localStorage.getItem(LocalStorage.Lang) || locale.value;
    ...
    const currentLang = ref<Language>(<Language> find(propEq('value',initialLangPref))(languages));

    watchEffect(()=> localStorage.setItem(LocalStorage.Currency,currentCurrency.value));
    watchEffect(()=> {
